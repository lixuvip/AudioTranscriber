...
| `arguments.engine` | arguments | 是 | App 转换后的脚本引擎名，例如 `funASR`, `vibeVoiceMLX`, `whisperMLX`, `qwen3ASR`。 |
| `arguments.model_id` | arguments | 是 | 转写模型 ID。Whisper MLX 可用 `mlx-community/whisper-large-v3-turbo` 等 MLX Whisper 模型；Qwen3 可带 dtype 后缀，例如 `Qwen/Qwen3-ASR-0.6B:dtype=float16`。 |
...
```json
{
  ...
  "available_engines": ["funASR", "vibeVoiceMLX", "whisperMLX", "qwen3ASR"]
}
```
...
- 依赖 `mlx-whisper`、`huggingface_hub` 和已缓存的 MLX Whisper 模型。
- 默认模型为 `mlx-community/whisper-large-v3-turbo`，也可使用本地 MLX Whisper 模型路径。
- 该引擎只做转写和时间戳分段，不提供说话人区分，也不读取声纹库。
- 健康检查和普通验证只检查依赖与缓存，不自动下载大模型；模型下载应由用户在 App 安装/下载流程中明确触发。
